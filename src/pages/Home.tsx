import React, { useEffect, useState, useRef, cloneElement, lazy, memo } from 'react';
import { Link } from 'react-router-dom';
import { ArrowRight, MapPin, Calendar, Users, Star, ChevronRight, Phone, X, MessageCircle, Check, Clock, ShieldCheck, Compass, Award, Heart, Mail, Send, Upload, Camera, ChevronDown, Menu, Home as HomeIcon, Info, Package, Image, MessageSquare, ChevronUp } from 'lucide-react';
...
const Home = () => {
  ...
  const [isLoadingTestimonials, setIsLoadingTestimonials] = useState(true); // Loading state for testimonials
  ...
  const heroImages = ["/waterfall.png", "/waterflower.png", "/Tiger.png", "/Mountain.png"];
  const handleFileChange = e => {
    ...
    if (file) {
      ...
      setPreviewUrl(null);
      // Reset avatar to default if file is cleared
      setFeedbackFormData(prev => ({
        ...prev,
        avatar: "/happy_customer_4.jpg"
      }));
  ...
  const handleFeedbackSubmit = async (e) => { // Added 'async' here
    e.preventDefault();
    setIsSubmitting(true);
    ...
    try {
      const reviewDataToSave = {
        ...
        location: feedbackFormData.location,
        text: feedbackFormData.text,
        rating: feedbackFormData.rating,
        avatar: feedbackFormData.avatar, // Use the avatar URL (default or data URL preview)
        timestamp: new Date().toISOString(), // Add a timestamp for sorting if needed
      };
      // Call the addReview function to write to Firebase
      const result = await addReview(feedbackFormData); // Assuming addReview is imported

      if (result.success) {
        // If Firebase write is successful, update local state and show success message
        setAllTestimonials(prev => [reviewDataToSave, ...prev]);
        setSubmitSuccess(true);
        ...
        // Handle error if Firebase write fails
        console.error("Error submitting feedback to Firebase:", result.error);
  ...
  useEffect(() => {
    const fetchTestimonials = async () => {
      setIsLoadingTestimonials(true); // Set loading state
      const result = await getReviews();
      if (result.success) {
        // Sort testimonials if needed (e.g., by timestamp, if you added one)
        setAllTestimonials(result.data.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))); // Example sort by timestamp descending
      } else {
        console.error("Failed to fetch testimonials:", result.error);
        // Optionally show an error message to the user
      }
      setIsLoadingTestimonials(false); // Unset loading state
    };
  ...
  return <div className="w-full">
    ...
    <section className="relative min-h-[100vh] overflow-hidden">
      ...
      <div className="relative z-20 container mx-auto px-4 sm:px-6 h-full flex flex-col justify-center py-10 md:py-0">
        <div className="max-w-3xl pt-28 sm:pt-32 md:pt-40 lg:pt-44 pb-16 sm:pb-20 md:pb-24 lg:pb-28">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-white opacity-0 animate-fadeInUp font-display leading-tight" style={{
          ...
          <p className="text-base sm:text-lg md:text-xl text-white mb-6 sm:mb-8 opacity-0 animate-fadeInUp max-w-2xl mt-4 sm:mt-6 text-center sm:text-left mx-auto sm:mx-0" style={{
          ...
          >
            <Link
              to="/packages"
              className="min-w-[160px] h-[44px] bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-md font-medium text-sm transition-all transform hover:scale-105 hover:shadow-lg flex items-center justify-center btn-hover-effect"
            >
            ...
            <Link
              to="/short-inquiry"
              className="min-w-[160px] h-[44px] bg-white hover:bg-gray-100 text-gray-900 px-4 py-2 rounded-md font-medium text-sm transition-all transform hover:scale-105 hover:shadow-lg flex items-center justify-center btn-hover-effect"
            >
              Quick Inquiry
            </Link>
          </div>
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 z-20 flex space-x-3">
        {heroImages.map((_, index) => <button key={index} onClick={() => setCurrentImageIndex(index)} className={`w-3 h-3 rounded-full transition-all ${index === currentImageIndex ? 'bg-green-400 scale-125' : 'bg-white/50 hover:bg-white/80'}`} aria-label={`Go to slide ${index + 1}`}></button>)}
      </div>
    </section>
    {/* Quick Info Section - Improved card design */}
    <section ref={observerRefs.quickInfo} className="bg-white py-12 sm:py-16 relative z-10">
      <div className="container mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
          <AnimateOnScroll animation="fade-up" delay={0.03} className="card-hover-effect h-full">
          ...
          <AnimateOnScroll animation="fade-up" delay={0.05} className="card-hover-effect h-full">
            <div className="bg-gray-50 rounded-xl p-6 shadow-md flex items-center transform transition-transform hover:-translate-y-2 duration-300 h-full lg:h-[140px]">
              <div className="bg-green-100 rounded-full p-4 mr-4 flex-shrink-0">
          ...
          <AnimateOnScroll animation="fade-up" delay={0.08} className="card-hover-effect h-full">
            <div className="bg-gray-50 rounded-xl p-6 shadow-md flex items-center transform transition-transform hover:-translate-y-2 duration-300 h-full lg:h-[140px]">
              <div className="bg-green-100 rounded-full p-4 mr-4 flex-shrink-0">
          ...
          <AnimateOnScroll animation="fade-up" delay={0.1} className="card-hover-effect h-full">
            <div className="bg-gray-50 rounded-xl p-6 shadow-md flex items-center transform transition-transform hover:-translate-y-2 duration-300 h-full lg:h-[140px]">
              <div className="bg-green-100 rounded-full p-4 mr-4 flex-shrink-0">
    ...
    <section ref={observerRefs.introduction} className="py-16 sm:py-20 md:py-24 bg-white">
      <div className="container mx-auto px-4 sm:px-6">
        <div className="flex flex-col md:flex-row gap-12 lg:gap-16 items-center">
          <AnimateOnScroll animation="fade-right" className="w-full md:w-1/2">
            <div className="relative">
              <div className="img-hover-zoom rounded-2xl overflow-hidden shadow-2xl">
                <img src="/bg1.png" alt="Beautiful Sri Lanka Coastline" className="w-full h-auto object-cover hover:shadow-2xl transition-shadow duration-300" />
              </div>
            ...
            <div className="grid grid-cols-3 gap-4 mt-4">
              <AnimateOnScroll animation="fade-up" delay={0.05} className="overflow-hidden rounded-xl shadow-md hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                <div className="img-hover-zoom h-32 sm:h-36 md:h-40">
                  <img src="/dambulla.png" alt="Sri Lanka Temple" className="w-full h-full object-cover hover:scale-110 transition-transform duration-500" />
                </div>
              </AnimateOnScroll>
              <AnimateOnScroll animation="fade-up" delay={0.1} className="overflow-hidden rounded-xl shadow-md hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                <div className="img-hover-zoom h-32 sm:h-36 md:h-40">
                  <img src="/train.png" alt="Sri Lanka Train Journey" className="w-full h-full object-cover hover:scale-110 transition-transform duration-500" />
                </div>
              </AnimateOnScroll>
              <AnimateOnScroll animation="fade-up" delay={0.15} className="overflow-hidden rounded-xl shadow-md hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                <div className="img-hover-zoom h-32 sm:h-36 md:h-40">
                  <img src="/wildlife.png" alt="Sri Lanka Wildlife" className="w-full h-full object-cover hover:scale-110 transition-transform duration-500" />
                </div>
              </AnimateOnScroll>
            </div>
          </AnimateOnScroll>
          <AnimateOnScroll animation="fade-left" delay={0.3} className="w-full md:w-1/2">
            ...
            <div className="space-y-4 mb-8">
              <AnimateOnScroll animation="fade-left" delay={0.1} className="flex items-start transform transition-transform hover:translate-x-2 duration-300">
                <Check size={20} className="text-green-600 mr-3 mt-1 flex-shrink-0" />
                <p className="text-gray-700 text-base">
                  Rich cultural heritage spanning over 2,500 years
                </p>
              </AnimateOnScroll>
              <AnimateOnScroll animation="fade-left" delay={0.15} className="flex items-start transform transition-transform hover:translate-x-2 duration-300">
                <Check size={20} className="text-green-600 mr-3 mt-1 flex-shrink-0" />
                <p className="text-gray-700 text-base">
                  8 UNESCO World Heritage Sites in a compact area
                </p>
              </AnimateOnScroll>
              <AnimateOnScroll animation="fade-left" delay={0.2} className="flex items-start transform transition-transform hover:translate-x-2 duration-300">
                <Check size={20} className="text-green-600 mr-3 mt-1 flex-shrink-0" />
                <p className="text-gray-700 text-base">
    ...
    <section ref={observerRefs.destinations} className="py-16 md:py-24 bg-gray-50">
      <div className="container mx-auto px-4 sm:px-6">
        <div className={`text-center mb-12 md:mb-16 transition-all duration-300 ${visibleSections.current.destinations ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-10'}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {destinations.map((destination, index) => <div key={index} className={`bg-white rounded-2xl overflow-hidden shadow-lg transition-all duration-300 hover:shadow-xl hover:-translate-y-2 ${visibleSections.current.destinations ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'}`} style={{
            transitionDelay: `${0.05 + index * 0.05}s`,
            willChange: 'transform, opacity'
          }}>
            <div className="relative h-64 overflow-hidden group">
              <img src={destination.image} alt={destination.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" loading="lazy" />
            ...
            <div className="p-6">
              <p className="text-gray-600 mb-4 text-base">
                {expandedDestinations[destination.id] ? destination.fullDescription || destination.description : `${destination.description.substring(0, 120)}...`}
              </p>
              <button onClick={() => toggleDestinationDetails(destination.id)} className="inline-flex items-center text-green-600 hover:text-green-800 font-medium text-sm group">
                {expandedDestinations[destination.id] ? 'Show Less' : 'Discover More'}{' '}
    ...
    <section ref={observerRefs.packages} className="py-16 md:py-24 bg-white">
      <div className="container mx-auto px-4 sm:px-6">
        <div className={`text-center mb-12 md:mb-16 transition-all duration-300 ${visibleSections.current.packages ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-10'}`}>
          <div className="inline-block bg-green-100 text-green-800 px-4 py-1 rounded-full text-sm font-medium mb-4">
            Travel Packages
          </div>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-4 text-gray-900 font-display">
        ...
        <div className="grid grid-cols-1 gap-10">
          <div className={`bg-white rounded-2xl overflow-hidden shadow-xl border border-gray-100 flex flex-col md:flex-row transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 ${visibleSections.current.packages ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'}`} style={{
            transitionDelay: '0.05s'
          }}>
            <div className="md:w-2/5 relative">
              <img src="/galle_fort.png" alt="14 Days Adventure" className="w-full h-64 md:h-full object-cover transition-transform duration-700 hover:scale-105" />
            ...
            <div className="md:w-3/5 p-6 md:p-8">
              ...
              <div className="flex items-center mb-6">
                <div className="flex">
                  {[1, 2, 3, 4, 5].map(star => <Star key={star} size={16} className={`${star <= 4 ? 'text-yellow-400 fill-current' : 'text-yellow-400'}`} />)}
                </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <Link to="/packages/14-days-13-nights" className="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-md text-base font-medium transition-all transform hover:scale-105 hover:shadow-md min-w-[140px] h-[44px] flex items-center justify-center">
                  View Details
                </Link>
              </div>
            </div>
          ...
          <div className={`bg-white rounded-2xl overflow-hidden shadow-xl border border-gray-100 flex flex-col md:flex-row transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 ${visibleSections.current.packages ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'}`} style={{
            ...
            <div className="md:w-2/5 relative">
              <img src="/beach1.png" alt="9 Days Adventure" className="w-full h-64 md:h-full object-cover transition-transform duration-700 hover:scale-105" />
            ...
            <div className="md:w-3/5 p-6 md:p-8">
              ...
              <div className="flex justify-between items-center">
                <div className="text-gray-900 font-bold text-xl"></div>
                <Link to="/packages/09-days-08-nights" className="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-md text-base font-medium transition-all transform hover:scale-105 hover:shadow-md min-w-[140px] h-[44px] flex items-center justify-center">
                  View Details
                </Link>
              </div>
            </div>
        ...
        <div className={`text-center mt-12 transition-all duration-300 ${visibleSections.current.packages ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} style={{
          transitionDelay: '0.15s'
        }}>
          <Link to="/packages" className="inline-flex items-center justify-center bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-md font-medium transition-all transform hover:scale-105 hover:shadow-lg min-w-[180px] h-[48px] text-base">
    ...
    <section ref={observerRefs.whyChoose} className="py-16 md:py-24 bg-gray-900 text-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {features.map((feature, index) => <div key={index} className="bg-gray-800 bg-opacity-50 p-8 rounded-2xl text-center hover:scale-105 hover:bg-gray-800 border border-gray-700 transition-all duration-300">
    ...
    <section ref={observerRefs.testimonials} className="py-16 md:py-24 bg-gray-50 relative">
      <div className="container mx-auto px-4 sm:px-6">
        <div className="text-center mb-12 md:mb-16">
          <div className="inline-block bg-green-100 text-green-800 px-4 py-1 rounded-full text-sm font-medium mb-4">
            Testimonials
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <div className="flex flex-wrap justify-center gap-4">
              <button onClick={() => setShowFeedbackForm(true)} className="inline-flex items-center justify-center bg-green-600 hover:bg-green-700 text-white px-5 py-3 rounded-md font-medium transition-all transform hover:scale-105 hover:shadow-md min-w-[220px] h-[52px] text-base">
              ...
              <button onClick={() => setIsTestimonialsExpanded(!isTestimonialsExpanded)} className="inline-flex items-center justify-center bg-gray-800 hover:bg-gray-900 text-white px-5 py-3 rounded-md font-medium transition-all transform hover:scale-105 hover:shadow-md min-w-[220px] h-[52px] text-base">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {allTestimonials.slice(0, isTestimonialsExpanded ? undefined : 3).map((testimonial, index) => <div key={index} className="bg-white p-8 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300">
            <div className="flex items-center mb-4">
              {[1, 2, 3, 4, 5].map(star => <Star key={star} size={16} className={`${star <= testimonial.rating ? 'text-yellow-400 fill-current' : 'text-gray-300'}`} />)}
            </div>
      ...
      {/* Feedback Form Modal */}
      {showFeedbackForm && <div className="fixed inset-0 bg-black bg-opacity-70 z-[200] flex items-center justify-center p-4 animate-fadeIn">
        <div className="bg-white rounded-xl shadow-xl w-full max-w-md relative animate-scaleIn max-h-[90vh] flex flex-col">
          <button onClick={closeFeedbackForm} className="absolute right-4 top-4 text-gray-800 hover:text-gray-900 transition-colors bg-gray-200 hover:bg-gray-300 rounded-full p-2 z-[210]" aria-label="Close feedback form">
            <X size={24} />
          </button>
              ...
              <div className="mb-4">
                ...
                <div className="flex items-center">
                  {[1, 2, 3, 4, 5].map(star => <button key={star} type="button" onClick={() => handleRatingChange(star)} className="focus:outline-none mr-1 transform transition-transform hover:scale-125">
                    <Star size={24} className={`${star <= feedbackFormData.rating ? 'text-yellow-400 fill-current' : 'text-gray-300'}`} />
                  </button>)}
                </div>
              </div>
              <div className="mb-6">
    ...
    <section ref={observerRefs.contactUs} className="py-16 md:py-24 bg-gray-900 text-white relative overflow-hidden">
      <div className="absolute inset-0 bg-cover bg-center opacity-20" style={{
        backgroundImage: "url('https://uploadthingy.s3.us-west-1.amazonaws.com/jh8EH1fsnWUQZd1GUr5t9h/nature.jpg')"
      }}></div>
      <div className="container mx-auto px-4 sm:px-6 relative z-10">
        <div className="text-center mb-12 md:mb-16">
          <div className="inline-block bg-green-900 bg-opacity-50 text-green-400 px-4 py-1 rounded-full text-sm font-medium mb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-stretch">
          ...
          <div className="h-full">
            <div className="bg-white text-gray-800 p-8 rounded-2xl shadow-2xl h-full flex flex-col">
              ...
              {contactSubmitSuccess ? <div className="bg-green-50 border border-green-200 text-green-700 px-6 py-8 rounded-lg mb-6 animate-fadeIn flex-grow flex flex-col items-center justify-center">
                <div className="flex items-center mb-4">
                ...
                <div className="mb-4">
                  ...
                  <input type="text" id="contact-name" name="name" value={contactFormData.name} onChange={handleContactChange} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500 text-base" placeholder="John Doe" required />
                </div>
                <div className="mb-4">
                  <label htmlFor="contact-email" className="block text-sm font-medium text-gray-700 mb-1">
                    Email Address
                  </label>
                  <input type="email" id="contact-email" name="email" value={contactFormData.email} onChange={handleContactChange} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500 text-base" placeholder="john@example.com" required />
                </div>
                <div className="mb-6 flex-grow">
                  <label htmlFor="contact-message" className="block text-sm font-medium text-gray-700 mb-1">
                    Message
                  </label>
                  <textarea id="contact-message" name="message" rows={3} value={contactFormData.message} onChange={handleContactChange} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500 text-base h-full min-h-[120px]" placeholder="How can we help you?" required></textarea>
                </div>
                <a href={`mailto:info@wintours.com?subject=Inquiry from Website&body=Name: ${contactFormData.name}%0D%0AEmail: ${contactFormData.email}%0D%0AMessage: ${contactFormData.message}`} className="w-full bg-green-600 hover:bg-green-700 text-white py-3 px-6 rounded-md font-medium flex items-center justify-center transition-colors text-base h-12">
    ...
    <section className="py-16 md:py-24 bg-green-600 text-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 text-center relative z-10">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          <Link to="/inquiry" className="bg-white text-green-600 hover:bg-gray-100 px-6 py-3 rounded-md font-medium transition-all transform hover:scale-105 hover:shadow-lg flex items-center justify-center w-44 h-12 text-base">
            Make an Inquiry
          </Link>
          <Link to="/contact" className="bg-black text-white hover:bg-gray-900 px-6 py-3 rounded-md font-medium transition-all transform hover:scale-105 hover:shadow-lg flex items-center justify-center w-44 h-12 text-base">
            Contact Us
          </Link>
        </div>
      </div>
    ...
    <button onClick={() => window.scrollTo({
      top: 0,
      behavior: 'smooth'
    })} className="fixed bottom-6 left-6 bg-green-600 text-white p-3 rounded-full shadow-lg z-40 md:hidden" aria-label="Back to top">
      <ChevronUp size={24} />
    </button>
  </div>;
};
